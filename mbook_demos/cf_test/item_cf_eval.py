...
# purchaseHistory15FilePath = '../data/purchase_history_15.csv'
...
#
#
# purchase_history = load_data(purchaseHistoryFilePath)
# purchase_history_15 = load_data(purchaseHistory15FilePath)
# books = load_data(booksFilePath)
#
#
# # Train the model using purchase_history_15
# def train_model(purchase_history_15):
#     global cosine_similarities, item_to_index, index_to_item, purchase_counts, sparse_purchase_counts, item_popularity
#     train_data, test_data = train_test_split(purchase_history_15, test_size=0.2, random_state=42)
#     purchase_counts = train_data.groupby(['book_id', 'user_id']).size().unstack(fill_value=0)
#     sparse_purchase_counts = sparse.csr_matrix(purchase_counts)
#     cosine_similarities = cosine_similarity(sparse_purchase_counts)
#     item_to_index = {item: index for index, item in enumerate(purchase_counts.index)}
#     index_to_item = {index: item for item, index in item_to_index.items()}
#     item_popularity = purchase_counts.sum(axis=1).to_dict()
#     print(f"item_to_index contains {len(item_to_index)} entries.")
#     return test_data
#
#
# test_data = train_model(purchase_history_15)
#
#
# def recommend_items(user_id, n=top_n):
#     user_history = purchase_history[purchase_history['user_id'] == user_id]['book_id'].values
#     if len(user_history) == 0:
#         print(f"User ID {user_id} has no purchase history")
#         return []
#
#     # Aggregate similarities for all items the user has interacted with
#     similarity_scores = np.zeros(len(item_to_index))
#     for book_id in user_history:
#         if book_id in item_to_index:
#             book_index = item_to_index[book_id]
#             similarity_scores += cosine_similarities[book_index]
#
#     purchased_indices = [item_to_index[book_id] for book_id in user_history if book_id in item_to_index]
#     similarity_scores[purchased_indices] = 0  # Remove already purchased items
#     recommended_indices = np.argsort(similarity_scores)[::-1][:n * 3]  # Get more candidates
#     recommended_items = [index_to_item[idx] for idx in recommended_indices if idx in index_to_item]
#     recommended_items = [item for item in recommended_items if item not in user_history]
#
